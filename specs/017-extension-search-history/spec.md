...
As the operator, I can open a History tab in the Full-time extension and see each recent LinkedIn Search run with the exact search text/keywords used, when it ran, its terminal status, and the result counters from that run.
...
**Independent Test**: Run two LinkedIn searches from the extension Search tab using different text, then open History and verify both runs appear with their query text, timestamp, status, raw LinkedIn result count, accepted count, rejected count, and duplicate count.
...
As the operator, I can compare full search queries and individual keywords/tags by frequency and raw LinkedIn results found so I can learn which searches produce more vacancies over time.
...
1. **Given** two runs for the same keyword with overlapping posts, **When** History shows keyword aggregates, **Then** total raw results equals the raw count from run A plus the raw count from run B and does not subtract duplicate outcomes.
...
- **FR-007**: The History view MUST include aggregate comparison by exact full query and by keyword token, including search frequency, total raw LinkedIn results, average raw results per run, accepted count, duplicate count, and latest run time.
- **FR-008**: Aggregates MUST count every run attempt/result occurrence in the selected period, even when later runs produce duplicate opportunities already known from prior runs.
- **FR-009**: The operator MUST be able to sort or filter history by date, status, query/keyword, and at least one productivity metric such as total raw results or average raw results.
- **FR-010**: A run detail affordance MUST expose existing run/candidate/opportunity diagnostics without leaking secrets, raw auth tokens, or provider errors that contain sensitive data.
- **FR-011**: Existing Full-time Search behavior, saved keyword badges, AI filters, career-page search controls, Jobs list, Gmail sending, and AI Field Assistant behavior MUST remain backward compatible.
- **FR-012**: The feature MUST NOT add, remove, or change Freelance web app data, outreach settings, WhatsApp/email provider settings, or lead workflows.
- **FR-013**: The implementation MUST handle historical runs missing raw result counts as unknown/unavailable and exclude them from raw-count averages unless the UI clearly labels the estimate.
- **FR-014**: The feature MUST preserve owner scoping so one user's search history and aggregates are not visible to another user.
...
- **SC-003**: The operator can identify the top five exact queries and top five keyword tokens by total raw LinkedIn results in under 30 seconds from the extension.
